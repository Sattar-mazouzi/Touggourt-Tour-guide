...
const MapView: React.FC<Props> = ({ 
  ...
  return (
    <div className={`w-full ${height} rounded-[40px] overflow-hidden shadow-2xl border border-white relative bg-slate-100 group animate-in zoom-in-95 duration-700`}>
      ...
      <div className={`absolute top-4 ${lang === 'ar' ? 'right-4' : 'left-4'} z-[1000] flex flex-col gap-2`}>
      ...
      {/* Places Count Label */}
      {places.length > 0 && (
         <div className={`absolute bottom-4 ${lang === 'ar' ? 'left-4' : 'right-4'} z-[1000]`}>
            <button 
              onClick={() => {
                const map = mapRef.current;
              ...
              className="bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-full text-[10px] font-black text-white uppercase tracking-widest shadow-lg flex items-center gap-2 active:scale-95 transition-all"
            >
